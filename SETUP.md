...
## 0. Creating a real app from this GitHub template

If this repository is used as a GitHub template, create the new repository first, clone it, then run Flutter's platform generation with your real project name and organization:

```bash
flutter create --project-name my_app --org com.yourcompany .
```

This sets the Dart project name and creates native package identifiers like:
...
2. Insert your required variables (e.g. `API_BASE_URL=https://api.example.com`).
3. These keys are now accessible in Dart via `dotenv.env['API_BASE_URL']`.
...
Verify that your Base URL pointing to staging/production is correctly initialized inside your globally injected network class.
   *(Tip: Read this via `dotenv.env['API_BASE_URL']` inside your network initializer!)*
